...
int compute_asift_keypoints(vector<float>& image, int width, int height, int num_of_tilts, int verb, vector< vector< keypointslist > >& keys_all, siftPar &siftparameters, vector< vector< pair<float, float> > >&ttrr)
...
{	
  ...
  ttrr.resize(num_tilt);
  for (tt = 1; tt <= num_tilt; tt++){
	  ttrr[tt - 1].resize(keys_all[tt - 1].size());
  }
...
#pragma omp parallel for private(rr)
      for ( int rr = 1; rr <= num_rot1; rr++ ) 
      {
		...
		ttrr[tt - 1][rr - 1] = make_pair(t , theta);
        theta = theta * 180 / PI;
